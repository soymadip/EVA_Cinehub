...
if query.data.startswith("file"):
    ...
    if str(grp_id) != str(grpid):
        ...
        return await query.answer('Try /connect')
...
if offset != "":
    ...
    btn.append(
            [InlineKeyboardButton(text=" CHECK MY PM ", url=f"https://t.me/CinemaHub_Manager01bot")]
    )
else:
    btn.append(
...
async def advantage_spell_chok(msg): query = re.sub( r"\b(pl(i|e) ?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|latest|br((o|u)h?) |^h(e|a)?(l) (o) |mal(ayalam)?|t(h)?amil|file|that|find|und(o) |kit(t(i|y)?)?o(w)?|thar(u)?(o) w?|kittum(o) |aya(k) (um(o) )?|full\smovie|any(one)|with\ssubtitle(s)?)", "", msg.text, flags=re.IGNORECASE) # plis contribute some common words query = query.strip() + " movie" search = msg.text g_s = await search_gagala(query) g_s += await search_gagala(msg.text) gs_parsed = [] if not g_s: k = await msg.reply("if you are requesting movie, then waait until ADMIN responses.") await asyncio.sleep(8) await k.delete() return regex = re.compile(r". (imdb|wikipedia). ", re.IGNORECASE) # look for imdb / wiki results gs = list(filter(regex.match, g_s)) gs_parsed = [re.sub( r'\b(-([a-zA-Z-\s])-\simdb|(-\s)?imdb|(-\s)?wikipedia|(|)|-|reviews|full|all|episode(s)?|film|movie|series)', '', i, flags=re.IGNORECASE) for i in gs] if not gs_parsed: reg = re.compile(r"watch(\s[a-zA-Z0-9_\s-()] ) |. ", re.IGNORECASE)
for mv in g_s: match = reg.match(mv) if match: gs_parsed.append(match.group(1)) user = msg.from_user.id if msg.from_user else 0 movielist = [] gs_parsed = list(dict.fromkeys(gs_parsed)) if len(gs_parsed) > 3: gs_parsed = gs_parsed[ 3] if gs_parsed for mov in gs_parsed: imdb_s = await get_poster(mov.strip(), bulk=True) # searching each keyword in imdb if imdb_s: hmm = InlineKeyboardMarkup( [ [ InlineKeyboardButton("️‍♂️ ️‍♂️", url=f" https://google.com/search?q={search} ") ] ] ) k = await msg.reply(f" .\nG - \n\n . ", reply_markup=hmm) return SPELL_CHECK[msg.message_id] = movielist btn = [[ InlineKeyboardButton( text=movie.strip(), callback_data=f"spolling#{user}#{k}", ) ] for k, movie in enumerate(movielist)] btn.append([InlineKeyboardButton(text="Close", callback_data=f'spolling#{user}#close_spellcheck')]) m = await msg.reply(f"Hey, {msg.from_user.mention}!\nI couldn't find anything related to that\nDid you mean any one of these?", reply_markup=InlineKeyboardMarkup(btn)) await asyncio.sleep(10) await bot.send_message(LOG_CHANNEL, f" New Request - {search}\n From -{msg.from_user.mention}")

async def manual_filters(client, message, text=False): group_id = message.chat.id name = text or message.text reply_id = message.reply_to_message.message_id if message.reply_to_message else message.message_id keywords = await get_filters(group_id) for keyword in reversed(sorted(keywords, key=len)): pattern = r"( |^|[^\w])" + re.escape(keyword) + r"( |$|[^\w])" if re.search(pattern, name, flags=re.IGNORECASE): reply_text, btn, alert, fileid = await find_filter(group_id, keyword)
